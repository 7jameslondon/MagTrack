...
def test_run_accuracy_sweep_smoke():
    df = run_accuracy_sweep(n_images=4, rng_seed=0, z_nm_choices=DEFAULT_Z_CHOICES)
    assert not df.empty
...
def test_default_methods_present():
    df = run_accuracy_sweep(n_images=3, rng_seed=1, z_nm_choices=DEFAULT_Z_CHOICES)
    methods = set(df["method"].unique())
    assert {"com_xy", "com_autoconv_xy"}.issubset(methods)


def test_reproducible_errors_with_seed():
    df1 = run_accuracy_sweep(n_images=3, rng_seed=123, z_nm_choices=DEFAULT_Z_CHOICES)
    df2 = run_accuracy_sweep(n_images=3, rng_seed=123, z_nm_choices=DEFAULT_Z_CHOICES)
...
def test_parameter_combinations_produce_expected_count():
    df = run_accuracy_sweep(
        ...
        rng_seed=42,
        z_nm_choices=DEFAULT_Z_CHOICES,
    )
...
def test_single_photon_choice_matches_default():
    df_default = run_accuracy_sweep(n_images=2, rng_seed=0, z_nm_choices=DEFAULT_Z_CHOICES)
