...
class TestAutoConvParaFit(unittest.TestCase):
    ...
    def test_auto_conv_para_fit_reaches_subpixel_accuracy(self):
        tolerance = 0.05
        for xp in self.xp_modules:
            ...
            centers_x, centers_y = magtrack.auto_conv_para_fit(stack, guess_x, guess_y)
    ...
    def test_auto_conv_para_fit_improves_over_auto_conv(self):
        for xp in self.xp_modules:
            stack = self._to_xp(xp, self.stack_np)
            expected_x = self._to_xp(xp, self.expected_x_np)
            expected_y = self._to_xp(xp, self.expected_y_np)
            ...
            refined_x, refined_y = magtrack.auto_conv_para_fit(stack, guess_x, guess_y)
    ...
    def test_auto_conv_para_fit_does_not_modify_inputs(self):
        for xp in self.xp_modules:
            ...
            magtrack.auto_conv_para_fit(stack, guess_x, guess_y)
