...
class TestQiRefineXY(unittest.TestCase):
    ...
    def test_qi_refine_xy_improves_initial_guess(self):
        for xp in self.xp_modules:
            ...
            initial_offsets = self._compute_offsets(
                xp, guess_x, guess_y, expected_x, expected_y
            )

            refined_offsets_np = self._to_numpy(xp, refined_offsets)
            initial_offsets_np = self._to_numpy(xp, initial_offsets)

            self.assertLess(
                float(refined_offsets_np.mean()),
                float(initial_offsets_np.mean()),
            )
            self.assertTrue(
                bool(np.all(refined_offsets_np <= initial_offsets_np + 1e-8)),
